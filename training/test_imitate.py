...
from combat_pilot import ENCODER, SCENARIOS, SIZE
...
class DemonstrationTests(unittest.TestCase):
    def write_data(self,path):
        ...
        (path/'outcomes.json').write_text(json.dumps([
            {'status':'cleared','initialHealth':2,'health':2},
            {'status':'cleared','initialHealth':2,'health':1},
            {'status':'budget-incomplete','initialHealth':2,'health':2}]))
