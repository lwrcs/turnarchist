...
export class Game {
  ...
  public loadGame = () => {
    console.log("🎮 loadGame() called - initializing game...");
    let canvas = document.getElementById("gameCanvas");
    Game.ctx = (canvas as HTMLCanvasElement).getContext("2d", {
      alpha: false,
    }) as CanvasRenderingContext2D;
    console.log("🎨 Canvas context created:", Game.ctx);
    ...
    console.log("📁 Resource URL types:", {
      tilesetUrl: typeof tilesetUrl,
      objsetUrl: typeof objsetUrl,
      mobsetUrl: typeof mobsetUrl,
      itemsetUrl: typeof itemsetUrl,
      fxsetUrl: typeof fxsetUrl,
      fontUrl: typeof fontUrl,
    });
    ...
    Game.tileset.onload = () => {
      resourcesLoaded++;
      console.log(
        "✅ Tileset loaded, resources:",
        resourcesLoaded,
        "/",
        NUM_RESOURCES,
      );
    };
    Game.tileset.onerror = (err) => {
      console.error(
    ...
    Game.objset.onerror = () => {
      console.error("❌ Failed to load objset:", objsetUrl);
    };
    ...
    Game.mobset.onerror = () => {
      console.error("❌ Failed to load mobset:", mobsetUrl);
    };
    ...
    Game.itemset.onload = () => {
      resourcesLoaded++;
      console.log(
        "✅ Itemset loaded, resources:",
        resourcesLoaded,
        "/",
        NUM_RESOURCES,
      );
    };
    Game.itemset.onerror = () => {
      console.error("❌ Failed to load itemset:", itemsetUrl);
    };
    ...
    Game.fxset.onerror = () => {
      console.error("❌ Failed to load fxset:", fxsetUrl);
    };
    ...
    Game.fontsheet.onload = () => {
      resourcesLoaded++;
      console.log(
        "✅ Fontsheet loaded, resources:",
        resourcesLoaded,
        "/",
        NUM_RESOURCES,
      );
    };
    Game.fontsheet.onerror = () => {
      console.error("❌ Failed to load fontsheet:", fontUrl);
    };
    ...
    let checkResourcesLoaded = () => {
      console.log(
        "🔍 Checking resources loaded:",
        resourcesLoaded,
        "/",
        NUM_RESOURCES,
      );
      if (resourcesLoaded < NUM_RESOURCES) {
        console.log("⏳ Resources not ready yet, checking again in 500ms...");
        window.setTimeout(checkResourcesLoaded, 500);
      } else {
        console.log("🎉 All resources loaded! Starting game initialization...");
  ...
  run = (timestamp: number) => {
    if (!this.runStarted) {
      console.log("🎬 Render loop started! First frame:", timestamp);
  ...
  recordGameStats = (stats: any) => {
    try {
      const gameState = createGameState(this);
      safeRecordGameStats({
        ...stats,
        gameState: gameState,
      });
      ...
      // Record stats without game state
      safeRecordGameStats(stats);
    }
  };
