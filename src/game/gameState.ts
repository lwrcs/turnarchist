    ...
    if (p.mapGroup !== undefined) {
      ...
      if (groupRooms.length) {
        if (
          ...
          console.log(
            "🧭 LOAD: Player resolved by id in group or first of group",
            { id },
          );
        }
      }
              ...
              if (game.rooms.length > 0) {
                ...
                if (savedLocal?.mapGroup !== undefined) {
                  const groupRooms = game.rooms
                    ...
                    .sort((a, b) => a.id - b.id);
                  console.log(
                    "🧭 LOAD: Candidate group rooms for local player:",
                    {
                  ...
                  if (groupRooms.length) {
                    if (
                      ...
                      groupAndIdRoom =
                        ...
                        groupRooms[0];
                      console.log(
                        "🧭 LOAD: Resolved by id within group or fallback to first of group",
                      );
                    }
                  }
                }
                ...
                console.log("🧭 LOAD: Resolved room flags & membership", {
                  ...
                  inLevelRooms: !!game.level?.rooms?.find(
                    (rr) => rr.globalId === resolvedRoom.globalId,
                  ),
