...
const itemToKind = (item: Item): ItemKind | null => {
  ...
  if (item instanceof WeaponPlague) return "weapon_plague";
...
export const registerBuiltinItemCodecsV2 = (): void => {
  ...
  type GenericItemKind = Exclude<ItemKind, "key" | "scroll" | "spellbook" | LightKind | WeaponKind | ShieldKind | SpecialKind>;
  const genericSave = (
    ...
    return {
      ...
      equipped,
      groundedNoAnimate: (item.groundedNoAnimate || undefined) as boolean | undefined,
    };
  };
      ...
      spawn: (value, room, _ctx) => {
        ...
        switch (kind) {
          ...
          case "weapon_ethereal":
            ...
            break;
          case "weapon_plague":
            item = new WeaponPlague(room, value.x, value.y);
            break;
          case "spellbook_page":
