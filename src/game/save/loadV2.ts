...
export const loadSaveV2 = async (game: Game, save: SaveV2): Promise<Result<void>> => {
  ...
  // Generate main path depths 0..maxRegen WITH population.
  // maxRegen is the deepest floor actually visited this run; it may exceed the current save depth
  // when the player went to lower floors then returned to a shallower one before saving.
  ...
  // For saves that have mainPathPlan but no maxGeneratedDepth (created between those two
  // additions), derive maxRegen from the deepest depth in the plan. For saves with neither
  // field, fall back to saveDepth (original behavior: regenerate only the current floor).
  const maxRegenFromPlan = save.worldSpec.mainPathPlan?.length
    ? Math.max(...save.worldSpec.mainPathPlan.map((p) => p.depth))
    : save.worldSpec.depth;
  const maxRegen = save.worldSpec.maxGeneratedDepth ?? maxRegenFromPlan;
  console.log(`[loadV2 debug] maxRegen=${maxRegen} maxGeneratedDepth=${save.worldSpec.maxGeneratedDepth} saveDepth=${save.worldSpec.depth}`);
  console.log(`[loadV2 debug] mainPathPlan=`, JSON.stringify(save.worldSpec.mainPathPlan));
  for (let depth = 0; depth <= maxRegen; depth++) {
    const planEntry = save.worldSpec.mainPathPlan?.find((p) => p.depth === depth);
    // Detect old saves where PNG generation failed at runtime and fell back to procedural,
    // but the plan incorrectly recorded kind="png". Procedural rooms always have roomX=-1
    // or roomY=-1 (BSP always produces a room at partition (0,0)); PNG rooms never do.
    const hasProceduraCoordInDelta =
      planEntry?.kind === "png" &&
      save.delta.rooms.some(
        (rd) =>
          rd.depth === depth &&
          (rd.pathId === "main" || rd.pathId === undefined) &&
          typeof rd.roomX === "number" &&
          rd.roomX < 0,
      );
    const genOverride = hasProceduraCoordInDelta
      ? ({ forceProcedural: true, pngAttemptedBeforeFallback: true } as const)
  ...
  // Log what rooms were generated at each depth to help diagnose coordinate mismatches.
  for (let d = 0; d <= maxRegen; d++) {
    const lvl = game.levels[d];
    if (lvl) {
      const mainRooms = lvl.rooms.filter((r) => r?.pathId === "main" || !r?.pathId);
      console.log(`[loadV2 debug] game.levels[${d}] (l.depth=${lvl.depth} genSource=${lvl.genSource} pngUrl=${lvl.pngUrl ?? "none"}): ${mainRooms.length} main-path rooms`);
      mainRooms.forEach((r) => console.log(`  depth=${r.depth} pathId=${r.pathId} roomX=${r.roomX} roomY=${r.roomY} mapGroup=${r.mapGroup}`));
    }
  }
  ...
  console.log(`[loadV2 debug] game.levels.length=${game.levels.length} after main-path gen`);
  for (let d = 0; d < game.levels.length; d++) {
    const lvl = game.levels[d];
    console.log(`  game.levels[${d}]: ${lvl ? `depth=${lvl.depth} rooms.length=${lvl.rooms?.length}` : "undefined"}`);
  }
  ...
  for (const sp of save.worldSpec.sidepaths) {
    ...
    const mainCountBefore = mergeLevel.rooms.filter((r) => r?.pathId === "main" || !r?.pathId).length;
    ...
    const mainCountAfter = mergeLevel.rooms.filter((r) => r?.pathId === "main" || !r?.pathId).length;
    console.log(`[loadV2 debug] sp merge: spDepth=${spDepth} pathId=${sp.pathId} mergeLevel.depth=${mergeLevel.depth} game.levels[spDepth]=${game.levels[spDepth] ? "defined" : "UNDEFINED!"} mainRooms before=${mainCountBefore} after=${mainCountAfter}`);
  }

  // Set active room list and rebuild map.
  game.rooms = activeLevel.rooms;

  console.log(`[loadV2 debug] game.levels state before allRooms construction:`);
  for (let d = 0; d < game.levels.length; d++) {
    const lvl = game.levels[d];
    if (lvl?.rooms) {
      const mainCount = lvl.rooms.filter((r) => r?.pathId === "main" || !r?.pathId).length;
      const spCount = lvl.rooms.filter((r) => r?.pathId && r.pathId !== "main").length;
      console.log(`  game.levels[${d}]: depth=${lvl.depth} mainRooms=${mainCount} spRooms=${spCount} total=${lvl.rooms.length}`);
    } else {
      console.log(`  game.levels[${d}]: ${lvl ? "no rooms array" : "undefined"}`);
    }
  }
  ...
  for (const rd of save.delta.rooms) {
    ...
    if (!candidate) {
      // Debug: show all allRooms at the same depth to diagnose matching failures
      const sameDepthRooms = allRooms.filter((r) => r.depth === rd.depth);
      console.error(`[loadV2 debug] Missing room: pathId=${rd.pathId} depth=${rd.depth} roomX=${rd.roomX} roomY=${rd.roomY} mapGroup=${rd.mapGroup}`);
      console.error(`[loadV2 debug] allRooms at depth=${rd.depth} (${sameDepthRooms.length} total):`);
      sameDepthRooms.forEach((r) => {
        const used = usedGeneratedRooms.has(r);
        console.error(`  ${used ? "[USED]" : "      "} depth=${r.depth} pathId=${r.pathId} roomX=${r.roomX} roomY=${r.roomY} mapGroup=${r.mapGroup} gid=${r.globalId}`);
      });
      return err({
