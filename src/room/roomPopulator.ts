...
export class Populator {
  ...
  private getPaddedTilesFrom(
    ...
    if (maxX < minX || maxY < minY) return tiles;
    return tiles.filter(
   ...
   */
  private getTilesWithRelaxedPadding(
    room: Room,
    tiles: Tile[],
   ...
   */
  private findCastleSidepathExitRoom(
    isSingleRoomSidepathMaze: boolean,
    isPngCastle: boolean,
  ): Room | null {
    if (
  ...
  populateRooms = () => {
    ...
    const isSingleRoomSidepathMaze =
      this.level.isMainPath !== true &&
      this.level.rooms.length === 1 &&
      this.level.rooms[0]?.type === RoomType.ROPECAVE &&
    ...
    const castleExitRoom = this.findCastleSidepathExitRoom(
      isSingleRoomSidepathMaze,
      isPngCastle,
    );

    // populate each room by environment (props only; enemies placed level-wide below)
    const populatedRooms: Room[] = [];
    this.level.rooms.forEach((room) => {
      ...
      if (room !== castleExitRoom) populatedRooms.push(room);
    });
          ...
          for (let x = start; step > 0 ? x <= end : x >= end; x += step) {
            if (exitRoom.roomArray[x]?.[topY] instanceof Wall) {
              const torch = this.shouldUseCandle(exitRoom)
                ? new PlacedCandle(exitRoom, exitRoom.game, x, topY)
                : new PlacedTorch(exitRoom, exitRoom.game, x, topY);
  ...
  populateByEnvironment = (room: Room, skipEnemies = false) => {
  ...
  addTrainingDownladder = (opts: SidePathOptions) => {
    ...
    const validTiles = this.getTilesWithRelaxedPadding(
      room,
      baseTiles,
      opts?.softMargin ?? this.level.generationOptions?.softMargin ?? 8,
    );
  ...
  addDownladder = (opts: SidePathOptions) => {
    ...
    if (this.addedDownladder) return;
    // Also scan the level for an existing sidepath DownLadder (covers reload/edge cases).
    for (const r of this.level.rooms) {
      for (let x = r.roomX; x < r.roomX + r.width; x++) {
        for (let y = r.roomY; y < r.roomY + r.height; y++) {
          const t = r.roomArray[x]?.[y];
          if (t instanceof DownLadder && (t as DownLadder).isSidePath) {
  ...
  private shouldUseCandle(room: Room): boolean {
    return (
      (room.depth >= 0 && room.depth <= 1) ||
      room.envType === EnvType.CASTLE ||
      room.envType === EnvType.DARK_CASTLE
    );
  }
    ...
    for (let x = room.roomX + 1; x < room.roomX + room.width - 1; x++) {
      for (let y = room.roomY + 1; y < room.roomY + room.height - 1; y++) {
        ...
        const torch = this.shouldUseCandle(room)
          ? new PlacedCandle(room, room.game, x, y)
          : new PlacedTorch(room, room.game, x, y);
  ...
  private getCavePocketPropsWithReducedResourceWeights() {
    return this.getPropsWithReducedResourceWeights(EnvType.CAVE_POCKET);
  }

  private getCavePropsWithReducedResourceWeights() {
    return this.getPropsWithReducedResourceWeights(EnvType.CAVE);
  }

  private getPropsWithReducedResourceWeights(envType: EnvType) {
    const props = environmentData[envType].props;
  ...
  private populateMagmaCaveEnvironment(room: Room) {
    ...
    const cappedNumProps = Math.min(
      numProps * 1.25,
      room.getEmptyTiles().length,
    );
    ...
    this.replaceEdgeWalls(room, ObsidianBlock, 0.75);

    // ADD: Enemies after props, based on remaining space
    this.addRandomEnemies(room);
  }

  private populateCastleEnvironment(room: Room, skipEnemies = false) {
    const numProps = this.getNumProps(room);
    this.addPropsWithClustering(room, numProps, room.envType, {
      falloffExponent: 2,
      baseScore: 0.1,
    ...
    if (x && y) {
      ...
      if (leftOpen) {
        const t = this.shouldUseCandle(room)
          ? new PlacedCandle(room, room.game, x - 1, y)
          : new PlacedTorch(room, room.game, x - 1, y);
  ...
  private addTorches(
    ...
    if (
      ...
      const t = this.shouldUseCandle(room)
        ? new PlacedCandle(room, room.game, placeX, placeY)
        : new PlacedTorch(room, room.game, placeX, placeY);
  ...
  private addDoorPlacedTorches(room: Room, x: number, y: number, doorDir: Direction) {
    ...
    const makeTorchOrCandle = (tx: number, ty: number) => {
      if (this.shouldUseCandle(room)) {
        const c = new PlacedCandle(room, room.game, tx, ty);
        c.applyWallDirection(dir);
        room.entities.push(c);
      } else {
        const t = new PlacedTorch(room, room.game, tx, ty);
  ...
  private addPlacedTorches(
    ...
    const place = (x: number, y: number, dir: Direction) => {
      if (this.shouldUseCandle(room)) {
        const c = new PlacedCandle(room, room.game, x, y);
        c.applyWallDirection(dir);
        room.entities.push(c);
      } else {
  ...
  private replaceEdgeWalls(
    ...
    const edgeWalls: Array<{ x: number; y: number }> = [];
    for (let x = room.roomX; x < room.roomX + room.width; x++) {
      for (let y = room.roomY; y < room.roomY + room.height; y++) {
        const tile = room.roomArray[x]?.[y];
        if (!(tile instanceof Wall)) continue;
        const info = room.wallInfo.get(`${x},${y}`);
        if (!info?.isInnerWall) continue; // only operate on inner walls
        ...
        const touchesFloor = neighbors.some(
          (n) => n instanceof Floor || n instanceof SpawnFloor,
        );
        if (touchesFloor) edgeWalls.push({ x, y });
      }
    }
    ...
    for (const pos of edgeWalls) {
      if (Random.rand() <= weight) {
        const { x, y } = pos;
        room.roomArray[x][y] = new Floor(room, x, y);
    ...
    for (let x = room.roomX; x < room.roomX + room.width; x++) {
      for (let y = room.roomY; y < room.roomY + room.height; y++) {
        ...
        if (!(tile instanceof Wall)) continue;
        const info = room.wallInfo.get(`${x},${y}`);
        if (!info?.isInnerWall) continue; // keep perimeter walls intact
        ...
        const neighborWalls = [up, down, left, right].filter(
          (n) => n instanceof Wall,
        ).length;
        if (neighborWalls === 0) {
          room.roomArray[x][y] = new Floor(room, x, y);
   ...
   */
  private buildEntryExclusionZone(
    room: Room,
   ...
   */
  private computeEntryPoint(room: Room): { x: number; y: number } | null {
    if (room.doors.length > 0 && room.level) {
      ...
      if (entryX >= 0) return { x: entryX, y: entryY };
    }
   ...
   */
  private computeExitPoint(room: Room): { x: number; y: number } | null {
    if (room.doors.length > 0 && room.level) {
      const distMap = this.computeRoomDistanceFromStart(room.level);
      let exitX = -1;
      let exitY = -1;
      ...
      if (exitX >= 0) return { x: exitX, y: exitY };
    }
  ...
  private pickTileFarFromEntry(
    ...
    if (tiles.length === 0) return null;
    if (!entry) {
      const idx = Math.floor(Random.rand() * tiles.length);
      const tile = tiles.splice(idx, 1)[0];
      return { x: tile.x, y: tile.y };
    }
    const entryDists = tiles.map(
      (t) => Math.abs(t.x - entry.x) + Math.abs(t.y - entry.y),
    );
    ...
    let exitDists: number[] | null = null;
    let maxExitDist = 0;
    if (exit) {
      exitDists = tiles.map(
        (t) => Math.abs(t.x - exit.x) + Math.abs(t.y - exit.y),
      );
    ...
    const weights = tiles.map((_, i) => {
      ...
      const exitProximity =
        exitDists && maxExitDist > 0 ? 1 - exitDists[i] / maxExitDist : 0;
   ...
   */
  private computeRoomDistanceFromStart(level: Level): Map<string, number> {
    ...
    while (queue.length > 0) {
      ...
      for (const door of r.doors) {
        ...
        if (!next || dist.has(next.globalId)) continue;
  ...
  private addEnemiesForLevel(
    ...
    for (const room of populatedRooms) {
      if (room === castleExitRoom) continue;
      let tiles = room.getEmptyTiles();
      // Exclude tiles around entry-side doors only — exit doors are left open for spawns.
      ...
      for (let j = 0; j < tiles.length; j++) {
        const entranceNorm =
          entryPoint && maxEntryDist > 0 ? entryDists[j] / maxEntryDist : 0.5;
        const exitProximity =
          exitDists && maxExitDist > 0 ? 1 - exitDists[j] / maxExitDist : 0;
        // Normalise to [0,1]: max of (norm²+ norm×1) = 2 when norm=1, proximity=1.
        const doorDistNorm =
          (entranceNorm * entranceNorm + entranceNorm * exitProximity) / 2;
    ...
    const availableEnemies = this.getAvailableEnemiesForRoom(
      populatedRooms[0],
      envType,
    );
    if (availableEnemies.length === 0) {
    ...
    for (let i = 0; i < totalEnemies; i++) {
      const pool = allTiles.filter(
        (e) => !usedTiles.has(`${e.room.globalId}:${e.tile.x},${e.tile.y}`),
      );
      if (pool.length === 0) break;

      const selectedEnemy = Utils.randTableWeighted(availableEnemies);
      if (!selectedEnemy?.class?.add) continue;

      const entityWeight = Math.max(
        0,
        Math.min(1, selectedEnemy.entityWeight ?? 0.5),
      );
      const enemyBlobOptions = this.resolveBlobOptions(selectedEnemy.blob);
      const useBlob =
        enemyBlobOptions && Random.rand() <= (enemyBlobOptions.chance ?? 1);
      let candidatePool = pool;
      if (useBlob && enemyBlobOptions) {
        const blobFiltered = pool.filter((e) => {
          const bf = this.getBlobFieldFromCache(
            e.room,
            enemyBlobOptions!,
            enemyBlobFieldCache,
          );
        ...
        if (blobFiltered.length > 0) candidatePool = blobFiltered;
      }
      ...
      let r = Random.rand() * totalScore;
      let chosen: (typeof scores)[0] | null = null;
      ...
      if (!chosen) chosen = scores[scores.length - 1];
      const { room, tile } = chosen;
      const x = tile.x;
      const y = tile.y;

      const chosenBlobField =
        useBlob && enemyBlobOptions
          ? this.getBlobFieldFromCache(
              room,
              enemyBlobOptions,
              enemyBlobFieldCache,
            )
  ...
  populateTreasure = (room: Room, rand: () => number) => {
    ...
    const chestRoll = Game.randTable(
      [1, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6, 7, 7, 8, 9, 10],
      rand,
    );
  ...
  populateUpLadder = (room: Room, rand: () => number) => {
    ...
    if (!t) return;
    room.roomArray[t.x][t.y] = new UpLadder(room, room.game, t.x, t.y);
  };
  ...
  populateDownLadder = (room: Room, rand: () => number) => {
    ...
    const desiredPad = room.level?.generationOptions?.softMargin ?? 8;
    const baseTiles = room.getEmptyTilesNotBlockingDoors();
    const ladderTiles = this.getTilesWithRelaxedPadding(
      room,
      baseTiles,
      desiredPad,
    );
    ...
    const numChests = Math.ceil(Random.rand() * 5) + 2;
    ...
    tiles = tiles.filter(
      (tile) => tile.x !== ladderPos.x || tile.y !== ladderPos.y,
    );
  ...
  private addEnvironmentalFeatures(room: Room, rand: () => number) {
    ...
    switch (room.type) {
      ...
      case RoomType.DUNGEON:
        ...
        if (GameplaySettings.ORGANIC_TUNNELS_FORCE) {
          ...
          if (
            ...
            this.level.environment.type === EnvType.CAVE_POCKET ||
      ...
      case RoomType.ROPECAVE:
        if (this.level.environment.type === EnvType.CAVE_POCKET) {
  ...
  private populateSingleRoomSidepathMaze(room: Room, rand: () => number) {
    ...
    const ropeUps: Array<{ x: number; y: number; t: UpLadder }> = [];
    const sideDowns: Array<{ x: number; y: number; t: DownLadder }> = [];
    for (let x = room.roomX; x < room.roomX + room.width; x++) {
      for (let y = room.roomY; y < room.roomY + room.height; y++) {
        const t = room.roomArray[x]?.[y];
        if (t instanceof UpLadder && (t as UpLadder).isRope) {
          ropeUps.push({ x, y, t: t as UpLadder });
        }
        if (t instanceof DownLadder && (t as DownLadder).isSidePath) {
          sideDowns.push({ x, y, t: t as DownLadder });
        }
      }
    }
    ...
    for (const d of sideDowns) {
      if (isCaveWithPockets) continue; // preserve all cave pocket downladders
    ...
    if (keepRopeUp && keepSideDown) {
      ...
      if (desiredExitEnv === null) {
        ...
        if (!isCaveWithPockets) {
          ...
          room.items = room.items.filter((it) => !(it instanceof Key));
          return;
        }
      ...
      const inPadded = (x: number, y: number): boolean =>
      ...
      const relocateTile = (
        ...
        room.roomArray[fromX][fromY] = new Floor(room, fromX, fromY);
        place();
      };
      ...
      if (!inPadded(keepRopeUp.x, keepRopeUp.y)) {
        ...
        if (pos) {
          relocateTile(keepRopeUp.x, keepRopeUp.y, () => {
            const up = new UpLadder(room, room.game, pos.x, pos.y);
            up.isRope = true;
            up.isSidePath = true;
      ...
      if (keyId !== undefined) {
        ...
        room.items = room.items.filter((it) => {
          ...
          const match = (it as Key).doorID === keyId;
          if (match && !kept) {
            kept = true;
            return true;
          }
        ...
        if (!kept) {
          ...
          const pos = room.getRandomEmptyPosition(tiles);
          if (pos) {
            const key = new Key(room, pos.x, pos.y);
            (key as Key).doorID = keyId;
            room.items.push(key);
          }
        }
      }
    ...
    const minConnectedNodes =
      room.envType === EnvType.CAVE || room.envType === EnvType.CAVE_POCKET
    ...
    const up = new UpLadder(room, room.game, entrance.x, entrance.y);
    up.isRope = true;
    up.isSidePath = true;
    ...
    if (placedKeyAndExit) {
      ...
      const key = new Key(room, keyEndpoint.x, keyEndpoint.y);
      dl.lockable.setKey(key);
      room.items.push(key);
    }

    // For CAVE: place 1-3 unlocked cave pocket downladders at node endpoints.
    if (room.envType === EnvType.CAVE && cavePocketCount > 0) {
      const cavePocketOpts: SidePathOptions = {
        caveRooms: 1,
        locked: false,
        terminal: true,
        noBoss: true,
        envType: EnvType.CAVE_POCKET,
        mapWidth: 18,
        mapHeight: 18,
        softMargin: 4,
      };
    ...
    if ((oreEndpointA || oreEndpointB) && (room.envType === EnvType.CAVE || room.envType === EnvType.CAVE_POCKET)) {
